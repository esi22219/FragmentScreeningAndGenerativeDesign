...
import datetime, os, time, random, re, resource, sys, argparse
...
from SyntOn.src.synthi.SyntOn import *
from pathlib import Path
from tqdm import tqdm
RDLogger.DisableLog('rdApp.*')  # most aggressive get rid of unkekulized mol (only thing that pops up)

 
def make_progress(iterable, total, desc):
        return tqdm(iterable, total=total, desc=desc, unit="line")
...
def main(inp, SynthLibrary, outDir, simTh, strictAvailabilityMode, nCores=-1, analoguesLibGen=False,
    ...
    if analoguesLibGen and nCores==-1:
        ...
        mid = str(inp).split("/")[-1].split(".")[0]
        outDir = Path(outDir)
        
        if outDir.is_dir():
            out_path = outDir / "allSynthons_out"
            out_1_Path = outDir / f"{mid}_out"
        else:
            out_path = outDir.with_name(f"_allSynthons_{mid}_out")
            out_1_Path = outDir.with_name(f"_{mid}_out")

        outAllSynthons = open(out_path, "w")
        outOnePath = open(out_1_Path, "w")
        print("#")
        print(out_path, out_1_Path)

        total_lines = countLines(inp)
        progress_iter = make_progress(open(inp), total_lines, desc="Fragmentation & BB availability")
...
def cli() -> int:
    ...
    if args.nCores == -1 or args.analoguesLibGen or args.enumerationMode:
        ...
        mid = str(args.input).split("/")[-1].split(".")[0]
        ...
        outNamesList = splitFileByLines(args.input, str(outDir) + "/" + str(mid), linesPerFile)
        print(outNamesList)
        fixed_main = partial(main, outDir=args.outDir, simTh=args.simTh, SynthLibrary=args.SynthLibrary,
                             strictAvailabilityMode=args.strictAvailabilityMode, nCores=args.nCores, Ro2Filtration=args.Ro2Filtration,
        ...
        with ProcessPoolExecutor(max_workers=nCores) as executor:
            for out in executor.map(fixed_main, outNamesList):
                finalLog.append(out)
        with open(out_1_Path, "w") as outOnePath:
            for inp in outNamesList:
                for line in open(inp + "_out"):
                    if line.strip():
                        outOnePath.write(line)
                os.remove(inp + "_out")
                # for line in open(str(outDir) + "/" + "allSynthons_out"):
                #     if line.strip():
                #         outAllSynthons.write(line)
                # os.remove(str(outDir) + "/" + "allSynthons_out")
