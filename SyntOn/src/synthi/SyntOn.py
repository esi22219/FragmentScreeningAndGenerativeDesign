# type: ignore while not installed here
...
from SyntOn.src.synthi.UsefulFunctions import *
...
class enumeration:
    def __init__(self, outDir, Synthons = None, reactionSMARTS = None, maxNumberOfReactedSynthons=6, MWupperTh=None, MWlowerTh=None,
        ...
        self.__marksCombinations = {'C:10': ['N:20', 'O:20', 'C:20', 'c:20', 'n:20', 'S:20'],
                                    ...
                                    'c:60':['C:70'], 'C:60': ['C:70'], 'N:40': ['C:30'] } # reaction matching setup
    ...
    def getReconstructedMols(self, allowedToRunSubprocesses=False, randomSeed=None, seed = (0,0), mainRun = True):
        ...
        if randomSeed:
            ...
            Pool = []
            for partner in self.__poliFuncBB + self.__biFuncBB + self.__monoFuncBB: # yo my nomenclature is here i mean cops lol
    ...
    def __prepSynthonsAndReactions(self, Synthons, reactionSMARTS, analoguesEnumeration): # mwant to be prep? why perp?
        if Synthons == None:
    ...
    def __PrepMolForReconstruction(self, molList, bivalentN=False): # list of Mol objects returned
        newList = []
        ...
        for mol in molList:
            mol = AddHs(mol)
            for atom in mol.GetAtoms(): # this is hte kicker
                if atom.GetAtomMapNum() != 0: # check for synthon reaction sites
                    ...
                    for neighbor in atom.GetNeighbors():
                        if neighbor.GetAtomicNum() == 1:
                            ...
                            if repl not in atomsForMarkingForDoubleBonds and replCount == 1:
                                ...
                                break
            mol = RemoveHs(mol) # type: ignore imported from rdmolops but not recognized
            newList.append(mol)
        if bivalentN: # does it again for Bivalent?
            for mol in molList:
                mol = AddHs(mol)
    ...
    def __countAndMergeActiveThreads(self, Pool):
        ...
        for p in Pool:
            if p[1].empty():
                ...
                print("Number of so far reconstructed molecules (may contain duplicates) = " + str(self.__genNonUniqMols))  # i am gonna end up jst dozing off gere anyway :(
                d_names, f_names, main_dir = listDir(self.__outDir)
                n = 0
                for file in f_names:
                    if "temp_" in file:
...
class fragmentation:    
    # start with testing and changing here, the print debugging is already there for easy repeated installs and checks
    # os.path.join(os.path.split(os.path.split(os.path.realpath(__file__))[0])[0], "config" , "SetupForMacrocycles.xml")
    #os.path.join(os.path.split(os.path.split(os.path.realpath(__file__))[0])[0], "config" , "Setup.xml")
    def __init__(self, fragmentationMode="use_all", reactionsToWorkWith = "R1-R13", maxNumberOfReactionCentersPerFragment = 3,
        ...
        if macroCycleSetupFile is None:
...
def fragmentMolecule(smiles, SyntOnfragmentor, simTh=-1):
    mol = readMol(smiles)
    if mol:
        RemoveStereochemistry(mol) # type: ignore
