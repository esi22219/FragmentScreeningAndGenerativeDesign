...
from synthi.SyntOn_Classifier import BBClassifier
from synthi.UsefulFunctions import *
...
def mainSynthonsGenerator(initSmi, keepPG=False, Classes=None, returnDict=False, returnBoolAndDict=False):
    solventsToIgnore = ["OC(=O)C(=O)O", "CC(=O)O", "OS(=O)(=O)O", "[O-]Cl(=O)(=O)=O", "OP(=O)(O)O", "OC(=O)C(F)(F)F",
                        "OS(=O)(=O)C(F)(F)F", "OC(=O)O", "[O-]S(=O)(=O)C(F)(F)F", "OC=O", r"OC(=O)/C=C\C(=O)O", "[O-]C(=O)C(F)(F)F",
                        r"OC(=O)/C=C/C(=O)O"]
    ...
    if initMol == None or initMol.HasSubstructMatch(query):
        ...
        #BBmarks = resources.files("synthi") / "config" / "BB_Marks.xml"
        BBmarks = os.path.join(os.path.split(os.path.split(os.path.realpath(__file__))[0])[0], "config" , "BB_Marks.xml")
        
        with open(BBmarks, "r", encoding="utf-8") as f:
            xml_in = f.read()

        xml_out = make_leaving_groups_explicit_in_xml(xml_in, charge_halide=True)
        BBmarks = str(Path(BBmarks).with_stem(Path(BBmarks).stem + "_updated"))
        with open(BBmarks, "w", encoding="utf-8") as f:
            f.write(xml_out)


        tree = ET.parse(BBmarks)
        BB_Marks = tree.getroot()
        MarksSetup = __getReactionSMARTS(BB_Marks) # why this and not libsmarts as this is causign errors (i get why tho)
        polyfunc = False
        polyfuncName = []
